...
// 获取专栏详情
export async function getArticleDetail(id: string): Promise<ArticleFull> {
  const kv = getKV();

  const key = articleKey.getKey(id);
  const result = await kv.get<Article>(key, 'json');
  ...
  return { ...result, views: stats?.views || 0, likes: stats?.likes || 0, isLiked };
}
